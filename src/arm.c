  ...
  case CAPTURE_VISION_INFO:
    if (arm_requested) {
      ...
      if (vision_result == VISION_READY_FOR_CAPTURE ||
        ...
        log_message(LOG_INFO, "Vision not requested, vision result = %d\n",
                    vision_result);
      }
    } else {
      log_message(LOG_INFO, "Arm not requested\n");
    }
    break;
